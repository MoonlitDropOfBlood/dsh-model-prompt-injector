...
- **真正的 rc.1+ 不兼容点（codec 缺 create，宿主/client 两处，1.1.1 修复）**：0.1.7-rc.1+ 的**两侧** Remote 注册表都要求 codec 携带 `create()` 工厂（旧宿主 ≤ 0.1.5-rc.3 只要求 `schema.parse`）：① client 侧缺 `create` 时 `ctx.remote.$mount` 抛 `"strict codec has no create() factory"` → client 模块加载失败（宿主半不受影响，RPC 照常通——误判盲区）；② **宿主侧 typert.host.js 的 codec 缺 `create` 时，启动路径 `validateTypertManifest`/`requireStrictCodec` 直接拒绝（报 `result codec has no create() factory`），typert 注册表构建中断，连带其它命名空间的远程方法失效（实测 `llm/listProviders` 挂掉、提供商列表为空）**。修复：宿主/client 所有 codec 同时带 `schema` 与 `create: () => schema`（同 dsh-agent-approval 1.7.x / dsh-token-stats 1.5.x）。**教训：宿主大版本升级时，typert.host.js 与 client.js 的 codec 契约必须两侧一起核对；验证工具 = 宿主 `dsh-typert-loader` 的 `validateTypertManifest(pkgName, manifest)` 可直接单测。**
